...
void traffic_gen_1(int pkgWordCount, int expectedTxPkgCnt, hls::stream<ap_uint<512> >& s_data_in)
{
#pragma HLS dataflow
     ...
     for (int i = 0; i < expectedTxPkgCnt; ++i)
     {
          for (int j = 0; j < pkgWordCount; ++j)
          {
               ...
               {
                    #pragma HLS UNROLL
                    s_data(i*64+63, i*64) = 0x1000000000000000 + j;
               }
...
void traffic_gen_2(int pkgWordCount, int expectedTxPkgCnt, hls::stream<ap_uint<512> >& s_data_in)
{
#pragma HLS dataflow
...
void consumeW(
    hls::stream<ap_uint<512> >& s_result_out, int feature_size
) {

    for (int i = 0; i < feature_size * 16; i++) {
...
void hls_bil_krnl(
               ...
               int basePort_send_1, 
               int expectedTxPkgCnt_1, 
               int baseIpAddress_send_1,
               int basePort_send_2, 
               int expectedTxPkgCnt_2, 
               int baseIpAddress_send_2,
...
#pragma HLS INTERFACE s_axilite port=basePort_send_1 bundle = control
#pragma HLS INTERFACE s_axilite port=expectedTxPkgCnt_1 bundle = control
#pragma HLS INTERFACE s_axilite port=baseIpAddress_send_1 bundle=control
#pragma HLS INTERFACE s_axilite port=basePort_send_2 bundle = control
#pragma HLS INTERFACE s_axilite port=expectedTxPkgCnt_2 bundle = control
#pragma HLS INTERFACE s_axilite port=baseIpAddress_send_2 bundle=control
...
// s_data_out_2 store the data of partial result of layer 1
static hls::stream<ap_uint<512> >    s_data_in;
#pragma HLS STREAM variable=s_data_in depth=512

static hls::stream<ap_uint<512> >    s_data_out_1;
#pragma HLS STREAM variable=s_data_out_1 depth=16
static hls::stream<ap_uint<512> >    s_data_out_2;
#pragma HLS STREAM variable=s_data_out_2 depth=512

static hls::stream<int >    stage_status;
#pragma HLS STREAM variable=stage_status depth=2

     ap_uint<16> sessionID [128];
     ap_uint<32> baseIPAddr [2];
     ap_uint<16> basePort [2];
     uint32_t expectedRxByteCnt = expectedRxPkgCnt * 1024;

     initialize_base_ip_port(baseIpAddress_send_1, basePort_send_1, baseIpAddress_send_2, basePort_send_2, baseIPAddr, basePort);
...
#pragma HLS dataflow
     ...
     openConnections(useConn_send, baseIPAddr, basePort, m_axis_tcp_open_connection, s_axis_tcp_open_status, sessionID);
     ...
     consumeW(s_data_in, expectedRxPkgCnt);

     traffic_gen_1( pkgWordCount_send, expectedTxPkgCnt_1, s_data_out_1);
...
/////////////////////////////////////// Compute ////////////////////////////////////////
    ...
    gather_embeddings(s_embedding_1, s_embedding_2, s_embedding_3, s_feature_in);
    ...
    dataTransform(start_flag, s_embedding_table, s_result1_node1, s_data_out_2);
////////////////////////////////////////////////////////////////////////////////////////

    //sendData( m_axis_tcp_tx_meta, m_axis_tcp_tx_data, s_axis_tcp_tx_status,sessionID, s_data_out_2, useConn, expectedTxPkgCnt, pkgWordCount);
    sendData( m_axis_tcp_tx_meta, m_axis_tcp_tx_data, s_axis_tcp_tx_status, sessionID, useConn_send, s_data_out_1, expectedTxPkgCnt_1, s_data_out_2, expectedTxPkgCnt_2, pkgWordCount_send);   
