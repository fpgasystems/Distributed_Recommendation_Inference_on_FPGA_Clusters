...
void openConnections(
     ...
     ap_uint<16>* basePort,
     hls::stream<pkt64>& m_axis_tcp_open_connection, hls::stream<pkt32>& s_axis_tcp_open_status, 
...
void sendData(hls::stream<pkt32>& m_axis_tcp_tx_meta, 
               ...
               ap_uint<16>* sessionID,
               int useConn,
               hls::stream<ap_uint<512> >& s_data_out_1,
               int expectedTxPkgCnt_1, 
               hls::stream<ap_uint<512> >& s_data_out_2,
...
void recvData_consumeData_bilateral(int expectedRxByteCnt, 
               hls::stream<pkt16>& s_axis_tcp_rx_meta, 
               hls::stream<pkt512>& s_axis_tcp_rx_data,
               hls::stream<ap_uint<16> >& rxPacketLength,
               hls::stream<ap_uint<512> >& s_data_in);

void recvDataBilateral(int expectedRxByteCnt, 
               hls::stream<pkt128>& s_axis_tcp_notification, 
               hls::stream<pkt32>& m_axis_tcp_read_pkg, 
               hls::stream<pkt16>& s_axis_tcp_rx_meta, 
               hls::stream<pkt512>& s_axis_tcp_rx_data,
...
#pragma HLS dataflow
     ...
     #pragma HLS PIPELINE II=1
          openConnection_pkt.data(31,0) = baseIpAddr[i];
                ...
                )
{
     int sentPkgCnt_1 = 0;
     int sentPkgCnt_2 = 0;
     int sentPkgCnt = sentPkgCnt_1 + sentPkgCnt_2;
     int expectedTxPkgCnt = expectedTxPkgCnt_1 + expectedTxPkgCnt_2;
     ...
     do{
          ...
          if (first_round & !s_data_out_1.empty() & !s_data_out_2.empty())
          {
          ...
          else
          {
               if (!s_axis_tcp_tx_status.empty())
               {
                    ...
                    if (resp.error == 0)
                    {
                         if (resp.sessionID==0 | resp.sessionID==2 | resp.sessionID==4 | resp.sessionID==6)
                         {
                              if (sentPkgCnt_1 < expectedTxPkgCnt_1-1)
                              {
                              ...
                              {
                              #pragma HLS PIPELINE II=1
                                   ap_uint<512> s_data = s_data_out_1.read();
                                   pkt512 currWord;
                         ...
                         {
                              if (sentPkgCnt_2 < expectedTxPkgCnt_2-1)
                              {
                                   tx_meta_pkt.data(15,0) = resp.sessionID;
                                   tx_meta_pkt.data(31,16) = pkgWordCount*(512/8);
                                   m_axis_tcp_tx_meta.write(tx_meta_pkt);
                              ...
                              {
                              #pragma HLS PIPELINE II=1
                                   ap_uint<512> s_data = s_data_out_2.read();
                                   pkt512 currWord;
                                   for (int i = 0; i < (512/64); i++) 
                                   {
                                        #pragma HLS UNROLL
                         ...
                         //sentPkgCnt = sentPkgCnt_1 + sentPkgCnt_2;
                         flag = (sentPkgCnt_2 < expectedTxPkgCnt_2) | (sentPkgCnt_1 < expectedTxPkgCnt_1);
                         ////////////////////////////////////////////////////////////
                    }
                    else
                    {
     ...
     do{
          if (!s_axis_tcp_rx_meta.empty() & !rxPacketLength.empty())
          {
               ...
               do{
                    ...
                    s_data_in.write(s_data); // store received data into temp_data, FIFO
                    /////////////////////////////////////////////////////////////////
               ...
               hls::stream<ap_uint<512> >& s_data_in)
{
     ...
     hls::stream<ap_uint<16> >    rxPacketLength;
     #pragma HLS STREAM variable=rxPacketLength depth=256
     ...
     recvData_consumeData_bilateral(expectedRxByteCnt, 
               s_axis_tcp_rx_meta, 
               s_axis_tcp_rx_data,
               rxPacketLength,
               s_data_in);
