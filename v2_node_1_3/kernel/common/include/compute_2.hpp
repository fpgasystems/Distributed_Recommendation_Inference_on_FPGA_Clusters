...
void init_weights<INPUT_SIZE, ROW_PER_PE1>(W_TYPE* weights_transpose_local) {
    ...
    for (int row = 0; row < ROW_PER_PE1 / 2; row++) {
        for (int i = 0; i < INPUT_SIZE / INTS_PER_W; i++) {
            ...
            reg_even.range(287, 256) = row_template_even[INTS_PER_W * i + 8];
            reg_even.range(319, 288) = row_template_even[INTS_PER_W * i + 9];
            reg_even.range(351, 320) = row_template_even[INTS_PER_W * i + 10];
            reg_even.range(383, 352) = row_template_even[INTS_PER_W * i + 11];

            reg_even.range(415, 384) = row_template_even[INTS_PER_W * i + 12];
            reg_even.range(447, 416) = row_template_even[INTS_PER_W * i + 13];
            reg_even.range(479, 448) = row_template_even[INTS_PER_W * i + 14];
            reg_even.range(511, 480) = row_template_even[INTS_PER_W * i + 15];
            ...
            reg_odd.range(287, 256) = row_template_odd[INTS_PER_W * i + 8];
            reg_odd.range(319, 288) = row_template_odd[INTS_PER_W * i + 9];
            reg_odd.range(351, 320) = row_template_odd[INTS_PER_W * i + 10];
            reg_odd.range(383, 352) = row_template_odd[INTS_PER_W * i + 11];

            reg_odd.range(415, 384) = row_template_odd[INTS_PER_W * i + 12];
            reg_odd.range(447, 416) = row_template_odd[INTS_PER_W * i + 13];
            reg_odd.range(479, 448) = row_template_odd[INTS_PER_W * i + 14];
            reg_odd.range(511, 480) = row_template_odd[INTS_PER_W * i + 15];
...
void gather_results_512PEs_200PE(
    ...
    for_each_item:
    for (int item = 0; item < BATCH_NUM * BATCH_SIZE; item++) {
        #pragma HLS pipeline II=25  // write 1 result_all per CC
...
void dataTransform(hls::stream<W_TYPE>& s_embedding_table, hls::stream<W_TYPE> & s_result_all, hls::stream<ap_uint<512> > & s_data_in){
    ...
    for_each_item:
    // in these packets, first 56*512 bits represent embedding table
    // last 25*512 bits represent partial result of layer 1
    // 896*32+400*32 = 56*512+25*512 = 81*512
    // for 5000*32 items, need 810,000 packets

    for (int item = 0; item < BATCH_NUM * BATCH_SIZE; item++) {
        for (int i = 0; i < 56; i++){
            #pragma HLS pipeline II=1
            ap_uint<512> s_data = s_embedding_table.read();
        ...
        for (int i = 0; i < 25; i++){
            #pragma HLS pipeline II=1
            ap_uint<512> s_data = s_result_all.read();
