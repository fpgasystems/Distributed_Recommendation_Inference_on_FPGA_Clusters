...
def simulationInput_Rx(hexstream, empty=True):
    ...
    lastDict = {0 : "FF", 1 : "01", 2 : "03", 3 : "07", 4 : "0F", 5 : "1F", 6 : "3F", 7 : "5F"}
    ...
    print(ip_header)
    #ipChecksumResults = hex(calculateChecksum(binascii.hexlify(ip_header), len(ip_header)))
    ipChecksumResults = hex(calculateChecksum(ip_header, len(ip_header)))
    ipChecksumResults = ipChecksumResults[2:]
    while (len(ipChecksumResults) % 4) != 0:
        ipChecksumResults = "0" + ipChecksumResults
    ip_header = "4500" + ipLength + "00000000FF06" + ipChecksumResults + "0A0A0A0A01010101"
    ip_header = ip_header.upper()
    print(ipChecksumResults)
    print(ip_header)
    mac_header = "0102030405060A0B0C0D0E0F0800"
    ...
    hexstream = mac_header + ip_header + udp_header + hexstream
    hexstream = hexstream.upper()
