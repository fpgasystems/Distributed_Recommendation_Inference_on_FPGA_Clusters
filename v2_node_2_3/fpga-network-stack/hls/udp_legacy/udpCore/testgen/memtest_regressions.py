...
reqRx.write( memlib.rxInput_noCS(SEQ1[0]) )
reqRx.write( memlib.rxInput_wrongCS(SEQ1[0]) )
reqRx.close()
respRx = open("rxGoldenOutput.dat", "w")
respRx.write( memlib.rxOutput(SEQ1[0]) )	# Once for the input stream with checksum
respRx.write( memlib.rxOutput(SEQ1[0]) )	# Once for the output stream without a checksum
respRx.close()
reqTx = open("txInput.dat", "w")
reqTx.write( memlib.txInput(SEQ1[1]) )
reqTx.close()
respTx = open("txGoldenOutput.dat", "w")
respTx.write( memlib.txOutput(SEQ1[1]) )
respTx.close()
