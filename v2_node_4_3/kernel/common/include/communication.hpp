...
void recvData_handshake(
               int basePort_recv_1, 
               int expectedRxByteCnt_1, 
               int expectedRxByteCnt_2, 
               hls::stream<pkt128>& s_axis_tcp_notification, 
               hls::stream<pkt32>& m_axis_tcp_read_pkg,
               hls::stream<ap_uint<16> >& rxPacketLength_1, 
               hls::stream<ap_uint<16> >& rxPacketLength_2, 
               ap_uint<16>* sessionID,
               hls::stream<int >& stage_status);

void recvData_consumeData_bilateral(
               int expectedRxByteCnt_1, 
               int expectedRxByteCnt_2, 
               ...
               hls::stream<ap_uint<16> >& rxPacketLength_1, 
               hls::stream<ap_uint<16> >& rxPacketLength_2, 
               hls::stream<ap_uint<512> >& s_data_in_1,
               hls::stream<ap_uint<512> >& s_data_in_2);

void recvDataBilateral(
               int basePort_recv_1, 
               int expectedRxByteCnt_1, 
               int expectedRxByteCnt_2, 
               hls::stream<pkt128>& s_axis_tcp_notification, 
...
void nnCompute(hls::stream<ap_uint<512> >& s_data_in, hls::stream<ap_uint<512> >& s_data_out, int pkgWordCount_send, int expectedTxPkgCnt);
                ...
                )
{
     ...
     do{
          ...
          if (first_round & !s_data_out.empty() &!stage_status.empty())
          //if (first_round & !s_data_out.empty())
          {
               
               for (int i = 0; i < useConn; ++i)
               {
               #pragma HLS PIPELINE II=1
               //if sessionID is not successfully initialized, what would happen
                    tx_meta_pkt.data(15,0) = 1 + 2 * index;
               ...
               if (!s_axis_tcp_tx_status.empty())
               {
                    ...
                    if (resp.error == 0)
                    {
                         ...
                         #pragma HLS PIPELINE II=1
                              ...
                              //std::cout << "\nSend data: " << currWord.data << std::endl;
               ...
               hls::stream<int >& stage_status)
{
     ...
     do{
          ...
          if (!s_axis_tcp_notification.empty())
          {
               ...
               ap_uint<16> session_ID = tcp_notification_pkt.data(15,0);
               ...
               if (length!=0)
               {
                    if (dstPort==basePort_recv_1)
                    {
                         pkt32 readRequest_pkt;
                         readRequest_pkt.data(15,0) = session_ID;
                         readRequest_pkt.data(31,16) = length;
                         m_axis_tcp_read_pkg.write(readRequest_pkt);

                         rxPacketLength_1.write(length);
                         rxByteCnt_1 = rxByteCnt_1 + length;

                         //if (flag){
                         sessionID[0] = session_ID;
                         int temp_stage = 1;
                         stage_status.write(temp_stage);
                         flag = false;
                         //}
                    }
                    else
                    {
                         ...
                         rxPacketLength_2.write(length);
                         rxByteCnt_2 = rxByteCnt_2 + length;
                    }
               }
               flag_recv = (rxByteCnt_2 < expectedRxByteCnt_2) | (rxByteCnt_1 < expectedRxByteCnt_1);
          }          
               ...
               hls::stream<ap_uint<512> >& s_data_in_2)
{
     ...
     do{
          flag = !rxPacketLength_1.empty() | !rxPacketLength_2.empty();
          if (!s_axis_tcp_rx_meta.empty() & flag)
          {
               s_axis_tcp_rx_meta.read();
               if (!rxPacketLength_1.empty())
               {
                    length = rxPacketLength_1.read();
                    bool lastWord = false;
                    do{
                         ...
                         for (int i = 0; i < (512/64); i++){
                              #pragma HLS UNROLL
                              s_data(i*64+63, i*64) = rx_data.data(i*64+63, i*64);
                         }
                         s_data_in_1.write(s_data); // store received data into temp_data, FIFO
                         /////////////////////////////////////////////////////////////////
               ...
               else
               {
                    ...
                    do{
                         ...
                         s_data_in_2.write(s_data); // store received data into temp_data, FIFO
                         /////////////////////////////////////////////////////////////////
                         lastWord = rx_data.last;
                    }while(lastWord == false);
               ...
               hls::stream<int >& stage_status)
{
     ...
     hls::stream<ap_uint<16> >    rxPacketLength_1;
     #pragma HLS STREAM variable=rxPacketLength_1 depth=256
     hls::stream<ap_uint<16> >    rxPacketLength_2;
     #pragma HLS STREAM variable=rxPacketLength_2 depth=256
     ...
     recvData_consumeData_bilateral(
               expectedRxByteCnt_1,
               expectedRxByteCnt_2,  
               s_axis_tcp_rx_meta, 
               s_axis_tcp_rx_data,
...
void nnCompute(hls::stream<ap_uint<512> >& s_data_in, hls::stream<ap_uint<512> >& s_data_out, int pkgWordCount_send, int expectedTxPkgCnt){
     ...
     do{
          if(!s_data_in.empty()){
               for(int j = 0; j < pkgWordCount_send; j++){
               #pragma HLS PIPELINE II=1
                    ap_uint<512> s_data = s_data_in.read();

                    ap_uint<512> s_data_tmp;
                    for (int i = 0; i < (512/32); i++){
                         #pragma HLS UNROLL
                         s_data_tmp(i*32+31, i*32) = s_data(i*32+31, i*32) + 0x00000001;
                         //s_data_tmp(i*8+7, i*8) = 0xff;
                    }
                    s_data_out.write(s_data_tmp);   
               }
               computePkgCnt++;
          }
     }while(computePkgCnt < expectedTxPkgCnt);
}